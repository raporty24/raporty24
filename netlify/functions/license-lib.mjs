...
export function response(status, body) {
  return { ...headers(status), body: JSON.stringify(body) };
}
...
export function normalizeProduct(value) {
  return String(value || DEFAULT_PRODUCT).trim().toLowerCase().replace(/[^a-z0-9_-]/g, "_").replace(/_+/g, "_").replace(/^_+|_+$/g, "") || DEFAULT_PRODUCT;
}
...
export async function parseBody(event) {
  ...
  try { return JSON.parse(event.body); } catch { throw new Error("Nieprawidłowy JSON w body requestu."); }
}
...
export function checkProduct(license, requestedProduct) {
  return normalizeProduct(license.product || DEFAULT_PRODUCT) === normalizeProduct(requestedProduct || DEFAULT_PRODUCT);
}
